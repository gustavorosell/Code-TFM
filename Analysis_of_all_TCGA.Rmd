...
head(table(samples.names.brca))
```

Design matrix:
```{r}
head(colnames(data.frame(sample=(samples.names.brca))), n=10)
...
brca.cpm<-cpm(xy.brca.disp)[o.brca[1:10],]
head(colnames(brca.cpm))
```
...
```{r}
coad.TF.list<-coad[unique(grep(paste(c(TF.list),collapse = "|"), rownames(coad))),]
```
...
head(table(samples.names.coad))
```

Design matrix:
```{r}
head(colnames(data.frame(sample=(samples.names.coad))), n=10)
...
head(table(samples.names.luad))
```

Design matrix:
```{r}
head(colnames(data.frame(sample=(samples.names.luad))))
...
luad.cpm<-cpm(xy.luad.disp)[o.luad[1:10],]
head(colnames(luad.cpm))
```
...
head(table(samples.names.prad))
```

Design matrix:
```{r}
head(colnames(data.frame(sample=(samples.names.prad))))
...
prad.cpm<-cpm(xy.prad.disp)[o.prad[1:10],]
head(colnames(prad.cpm))
```
